...
export default function EcommerceFilters({ open, onClose }: Props) {
  ...
  const fetchBrandsList = async () => {
    try {
      ...
      const itemBrand = list.data.map((item: any) => item.name);

      if (itemBrand.length > 0 && BRAND_OPTIONS.length === 0) {
        BRAND_OPTIONS.push(...itemBrand);
      }
    } catch (error) {
  ...
  const fetchCategoriesList = async () => {
    try {
      ...
      const itemCategory = list.map((item: any) => item.name);

      if (itemCategory.length > 0 && CATEGORY_OPTIONS.length === 0) {
        CATEGORY_OPTIONS.push(...itemCategory);
      }
    } catch (error) {
